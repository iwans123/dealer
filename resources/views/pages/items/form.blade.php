    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
            <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                <form class="space-y-6" method="POST" action="{{ isset($item) ? route('item.update',$item->id) : route('item.store') }}">
                    @csrf
                    <h5 class="text-xl font-bold uppercase text-gray-900 dark:text-white">Add Item</h5>
                    <div class="mb-6">
                        <label for="name_item" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Item name</label>
                        <input type="text" id="name_item" name="name_item" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ isset($item) ? $item->name_item : '' }}" placeholder="xxxxxx" required>
                    </div>
                    <div class="mb-6">
                        <label for="path_number" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Path number</label>
                        <input type="text" id="path_number" name="path_number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ isset($item) ? $item->path_number : '' }}" placeholder="xxxxxx" required>
                    </div>
                    <div class="mb-6">
                        ...
                        <div class="flex">
                            <select id="category" name="category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                <option value="{{ isset($item) ? $item->category_id : '' }}">{{ isset($item) ? $item->category->name : 'Choose a category' }}</option>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}">{{ $category->name }}</option>
                                @endforeach
                            </select>
                            <button type="button" data-modal-target="add-modal" data-modal-toggle="add-modal" class="border rounded-lg ml-2 px-4 bg-green-700 hover:bg-green-600">
